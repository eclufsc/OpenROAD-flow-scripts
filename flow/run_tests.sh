...
while IFS= read -r tech_design; do
    ...
    if [ -s logs/"$tech_design"/epl2/5_2_route.json ]; then
        if [ $(jq -r ".detailedroute__flow__errors__count" logs/"$tech_design"/epl2/5_2_route.json) -eq 0 ]; then
            if [ ! -s logs/"$tech_design"/epl2/6_report.json ]; then
                make DESIGN_CONFIG=./designs/"$tech_design"/config.mk FLOW_VARIANT=epl2 GPL_ROUTABILITY_DRIVEN=0 GPL_TIMING_DRIVEN=0 EPL_PLACE=1
            fi
        fi
    fi
