...
puts "-------------------RUNING GRCMO FLOW 4 TCLLLL-----------------"
...
proc global_route_helper { } {
  ...
  if { $result != 0 } {
    if { !$::env(GENERATE_ARTIFACTS_ON_FAILURE) } {
      write_db $::env(RESULTS_DIR)/5_1_grcmo_grt-failed.odb
      error $errMsg
    }
    write_sdc -no_timestamp $::env(RESULTS_DIR)/5_1_grcmo_grt.sdc
    write_db $::env(RESULTS_DIR)/5_1_grcmo_grt.odb
    return
  }
  ...
  write_guides $::env(RESULTS_DIR)/grcmo_route.guide
  write_db $::env(RESULTS_DIR)/5_1_grcmo_grt.odb
  write_sdc -no_timestamp $::env(RESULTS_DIR)/5_1_grcmo_grt.sdc
}
