...
load_design 6_1_grcmo_fill.odb 6_1_grcmo_fill.sdc
...
write_db $::env(RESULTS_DIR)/6_grcmo_final.odb

# Delete routing obstructions for final DEF
source $::env(SCRIPTS_DIR)/deleteRoutingObstructions.tcl
deleteRoutingObstructions

write_def $::env(RESULTS_DIR)/6_grcmo_final.def
write_verilog $::env(RESULTS_DIR)/6_grcmo_final.v \
...
if {
  ...
  # Write Spef
  write_spef $::env(RESULTS_DIR)/6_grcmo_final.spef
  file delete $::env(DESIGN_NAME).totCap

  # Read Spef for OpenSTA
  read_spef $::env(RESULTS_DIR)/6_grcmo_final.spef
